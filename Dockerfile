...
RUN echo 'Hiasop' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/Hiasop.sh

RUN echo 'cd ' >>/Hiasop.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Hiasop.sh

RUN echo 'cd /noVNC-1.2.0' >>/Hiasop.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Hiasop.sh

RUN chmod 755 /Hiasop.sh
